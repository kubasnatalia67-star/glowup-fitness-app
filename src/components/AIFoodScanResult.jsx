  ...
  return (
    <>
      {foodAnalysisLoading && !foodResult && (
        <div className="toast-pop glow-card fixed inset-x-3 bottom-24 z-[66] mx-auto max-w-md p-4 text-white">
      ...
      {foodResult && foodPhoto && (
        <div className="toast-pop glow-card fixed inset-x-3 bottom-24 z-[65] mx-auto max-w-md p-4">
          <div className="flex gap-4">
            <img
              src={foodPhoto}
              alt="Фото їжі"
              className="h-24 w-24 rounded-2xl object-cover shadow-lg shadow-black/20"
            />
            <div className="min-w-0 flex-1">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-lg font-bold">🍽 {foodResult.name || foodResult.dish}</h3>
                  <p className="text-pink-300">{foodResult.calories} ккал</p>
                </div>
                <button
                  type="button"
                  onClick={onCloseResult}
                  className="tap-anim rounded-full bg-white/10 px-3 py-1 hover:bg-white/15"
                >
                  x
                </button>
              </div>
              {"protein" in foodResult ? (
                <div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs">
                  <div className="rounded-xl bg-white/10 p-2">
                    <p className="opacity-70">Білки</p>
                    <p className="font-bold">{foodResult.protein}g</p>
                  </div>
                  <div className="rounded-xl bg-white/10 p-2">
                    <p className="opacity-70">Жири</p>
                    <p className="font-bold">{foodResult.fat}g</p>
                  </div>
                  <div className="rounded-xl bg-white/10 p-2">
                    <p className="opacity-70">Вуглеводи</p>
                    <p className="font-bold">{foodResult.carbs}g</p>
                  </div>
                </div>
              ) : (
                <>
                  <p className="mt-2 text-sm text-white/70">{foodResult.ingredients}</p>
                  <p className="mt-1 text-xs text-white/50">{foodResult.macros}</p>
                </>
              )}
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <button
                  ...
                  className="tap-anim rounded-2xl bg-gradient-to-r from-pink-500 to-orange-400 px-4 py-3 text-sm font-black text-white shadow-lg shadow-pink-500/25 hover:-translate-y-0.5"
                >
                  Додати в щоденник
                </button>
                <button
      ...
      {foodAnalysisError && (
        <div className="toast-pop error-card fixed inset-x-3 bottom-24 z-[65] mx-auto max-w-md rounded-3xl border border-rose-400/30 p-4 text-white shadow-2xl shadow-rose-950/30">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  ...
                  className="tap-anim rounded-full bg-white/10 px-3 py-1 text-sm hover:bg-white/15"
                >
                  x
                </button>
              </div>
